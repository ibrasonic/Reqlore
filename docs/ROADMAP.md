...
**Phase 9 — Reliability, gap-closure, AAA polish:** ✅ complete (2026-06-09). **1368/1368 unit tests pass, 239 skipped** (skips gate optional extras: `[h3]`, `[impersonate]`, `[websocket]`, `[schedule]`, `[a11y]`). Three internal plans drove this phase: **RELIABILITY_PLAN** (component-health matrix, WCAG-AAA structural matrix, error-path coverage, long-session resilience), **SCANNER_GAP_PLAN** (16 active-scanner gaps closed — smuggling, sequencer auto-feed, forced-browsing, GraphQL batching, deserialisation, web-cache deception, resumable scans, plus the "Why not fired" coverage page), and **INTRUDER_ENHANCEMENTS** (Phase 0 AAA template fixes + Phase 1 operator control — pause / resume / cancel / server-driven auto-refresh; Phases 2-4 results-triage and export still in progress).

**Post-Phase-9 continuous shipping:** ongoing (2026-06-10 → 2026-07-06 and continuing). Unit suite now at **2883 passed, 368 skipped** (+1515 tests since Phase 9 close). Feature additions grouped by module:

- **DOM Hunter (new module, `reqlore.dom_hunter`)** — DOM-XSS source/sink tracer shipped as a bundled, Mozilla-signed Firefox WebExtension plus a server-side bridge. Per-source tagged canaries with real attribution (hash / postMessage / window.name / referrer / search / cookie / pathname / storage, in precedence order); proxy-side `document.referrer` auto-inject; 7 sink wrappers (innerHTML/outerHTML/insertAdjacentHTML, script.src/iframe.src/anchor.href, `setAttribute(on*)`, `document.write*`, `eval`, `setTimeout/Interval` strings, `Function` ctor). `reqlore browser --project` auto-downloads Firefox Dev Edition when needed so the sideloaded XPI is honored; managed-policy + XPI-sideload run in parallel for corporate-IT environments. Alt+7 keybinding.
- **Sequencer** — Burp-Sequencer parity pass: live token capture (paste target request, run for N samples), full deep statistical battery (bit-level frequency / correlation / runs / longest-run / char-position per bit), plain-English "random / NOT random" verdict on the results page, and a Send-to-Sequencer action on every history row.
- **History workbench** — column-disclosure filter menus on every filterable `<th>` (methods, statuses as buckets AND exact codes OR'd together, engines, host exact/contains, URL regex, length range, duration range); whole table wrapped in a single `<form role="search">` so Enter commits every column at once; 44×44 px targets (SC 2.5.5 AAA); Escape-to-close-with-focus-restore; open-one-closes-others. Live auto-refresh for new requests (WCAG AAA strict — `lastAnnouncedCount` guard so the live region only speaks on real change).
- **Server-side find-in-body** — WCAG AAA-clean (no-JS) `<form>`-based Find across History request+response, Repeater, Intercept, Scanner evidence+payload, WebSocket, and Macros. One canonical Find box per workbench, placed above the body panes.
- **Decoder + History decode** — smart decode toggle, exhaustive error handling with binary-content warnings, full encoding coverage, flexible input path from History.
- **Intruder** — per-position payload-source dropdowns for pitchfork / clusterbomb with the dropdown count auto-matching `§` marker-pair count; streaming payload sources with server-path wordlist (no size cap); real file upload for wordlist source; per-payload JWT signing via a new `jwt:` processor; silent-worker-crash surfacing; template hygiene (drops stale `Content-Length` / `Transfer-Encoding`).
- **JWT workbench** — smart public-key input on the RS→HS key-confusion action: accepts PEM (unchanged behaviour), single JWK, full JWKS document, or JWKS URL (fetched through Reqlore's logged HTTP path with `http`/`https`-only scheme allow-list, `follow_redirects=False`, 10 s timeout, 1 MB body cap, 128 KB paste cap). Selects by token `kid` else first RSA key; converts to SPKI PEM internally; signs via direct HMAC (bypassing PyJWT's `InvalidKeyError` asymmetric-key block — the exact operation a vulnerable server performs).
- **Proxy** — auto-advance to next pending intercept after a decision; mitmproxy CA now mirrored into `mitmproxy-ca.pem` so HTTPS interception works on first launch.
- **Match-and-replace** — Quick Presets for common pen-test rewrites.
- **Docs overhaul** — six-phase rewrite: per-module pages under `docs/modules/`, slimmed `USAGE.md`, moved internal dev plans, and refreshed all cross-cutting reference pages.
- **Release housekeeping** — relicensed to Apache-2.0; added GitHub Actions CI; filled `SECURITY.md` reporting details. Cross-cutting security-audit hardening pass (HIGH 5/5, MEDIUM 12/14, LOW 7/13 resolved).
