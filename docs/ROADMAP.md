...
**Phase 3 — Scanner foundation:** ✅ complete (2026-06-07). 115/115 unit tests pass. All 17 blueprint routes serve 200; `weblore scan` + `weblore report` CLI both work end-to-end.
...
**Phase 5 — Advanced / power tools:** ✅ complete (2026-06-07). 215/215 unit tests pass. All 26 blueprint routes serve 200; `weblore run` YAML/JSON job runner verified end-to-end (set → assert → scan → report).
...
- [x] `weblore.cli` entry point (`ui`, `proxy`, `init`, `both`)
- [x] `weblore.config` + per-project SQLite settings
- [x] `weblore.storage.Project` — schema, blob compression, thread-safe
- [x] `weblore.engines` — Request/Response dataclasses, Timings
- [x] `weblore.engines.httpx_engine`
- [x] `weblore.engines.raw_engine`
- [x] `weblore.engines.curl_render`
- [x] `weblore.proxy.ca` — CA gen + cert export (cryptography lib)
- [x] `weblore.proxy.mitm` — wraps mitmproxy DumpMaster in a thread
- [x] `weblore.proxy.rules` — match rules
- [x] `weblore.a11y` — contrast helpers, response summariser, copy-as renderers
- [x] `web/templates/base.html` — skip-link, live region, themes, verbosity
- [x] `web/static/weblore.css` — accessible defaults, three themes
- [x] `web/static/weblore.js` — progressive enhancement only
...
- [x] Finding model (CWE / OWASP / CVSS band) — `weblore.scanner.findings`
- [x] Passive scanner with 12 built-in rules — `weblore.scanner.passive`
...
- [x] Plugin API + loader — `weblore.plugins`
  - drop-in `.py` files in `~/.weblore/plugins/`
...
- [x] CLI: `weblore scan` and `weblore report`
...
- [x] Active scanner (`weblore.scanner.active`) — 8 built-in checks:
...
- [x] GraphQL workbench (`weblore.graphql` + `/graphql/`) — introspection, schema flatten, query runner
- [x] WebSocket workbench (`weblore.websocket` + `/ws/`) — sync `websockets` client, transcript persistence via `project_state` KV
- [x] SAML inspector (`weblore.saml` + `/saml/`) — POST/Redirect bindings (raw-DEFLATE via `-zlib.MAX_WBITS`), unsigned/weak-algo/missing-AudienceRestriction/missing-NotOnOrAfter audits
- [x] CSRF + Clickjacking PoC generator (`weblore.poc` + `/poc/`) — form / fetch flavours, HTML-escaped lure overlay
- [x] Session-handling macros (`weblore.macros` + `/macros/`) — `{{var}}` substitution, header/regex/JSON-path capture, persisted in `project_state`
- [x] 33 new tests (9 active scanner + 11 phase4 modules + 13 web smoke)
- [x] CLI unchanged — `weblore scan` / `weblore report` still work
- [x] All 148 tests pass; all 22 blueprint routes return 200

## Phase 5 — Advanced / power tools ✅

- [x] Sequencer (`weblore.sequencer` + `/sequencer/`) — Shannon entropy per char + per token, per-position breakdown, Hamming distance over consecutive tokens, longest-run detection, weak-position flagging
- [x] OAST (`weblore.oast` + `/oast/`) — local in-process HTTP callback receiver on `127.0.0.1:<random>`, per-token routing, in-memory ring of 5000 interactions, plus `interactsh_poll()` stub for opt-in remote polling
- [x] HTTP/2 frame tool (`weblore.h2_tool` + `/h2/`) — parse hex → typed frames (DATA/HEADERS/SETTINGS/PING/GOAWAY/RST_STREAM/WINDOW_UPDATE/CONTINUATION/PRIORITY/PUSH_PROMISE), preface detection, frame builders for SETTINGS / PING / GOAWAY / RST_STREAM / WINDOW_UPDATE
- [x] HTTP/3 engine (`weblore.engines.h3_engine`) — thin synchronous wrapper around `aioquic` (optional `[h3]` extra); availability flag + safe "install with pip install weblore[h3]" fallback when missing
- [x] Request-smuggling helpers (`weblore.smuggling` + `/smuggling/`) — CL.TE / TE.CL / TE.TE payload generators with downloadable bytes; timing-based detect helper using a sender callable
- [x] curl_cffi JA3/JA4 engine (`weblore.engines.curl_cffi_engine`) — optional `[impersonate]` extra; supports `chrome120`/`chrome119`/`safari17_0`/`firefox109` etc.; safe error response when missing
- [x] CLI runner (`weblore run jobs/job.yaml`) — YAML (optional `[yaml]` extra) or JSON; step types `request`, `scan`, `active`, `report`, `set`, `assert`, `sleep`; `{{var}}` substitution; header/JSON-path capture; `--strict` mode
- [x] Plugin SDK (`weblore.plugins_sdk`) — `make_info`, `make_passive_rule`, `CopyAsHandler`, `assert_compatible`
...
- [x] `weblore run` verified end-to-end (set → assert → scan → report → OK 158 ms)
...
- [x] OAST + Macros + Active scanner cross-flow: new `OASTSSRFCheck` (`weblore.scanner.active.OASTSSRFCheck`) injects the running receiver's callback URL into every query/form parameter, polls the OAST log for ~600 ms, and escalates to a `CWE-918` high-severity finding when a hit is observed
  - opt-in: only runs when `ActiveOptions.oast` is set; the scanner blueprint auto-wires `current_app.extensions["weblore_oast"]` when the receiver is running
...
- [x] axe-core a11y CI gate — `weblore/tests/a11y/test_axe_smoke.py` boots the app via `wsgiref` and runs axe on all 26 routes; gated behind the new `[a11y]` extra (`playwright` + `axe-playwright-python`) so the default `pytest weblore/tests/unit` run is unaffected
...
- [x] HAR 1.2 importer (`weblore.har`) — stdlib-only parser; builds raw request/response bytes per entry; new CLI subcommand `weblore import-har --project <p> <file.har>`; entries land with `engine="har"` and `flags="imported"`
- [x] Param-miner (`weblore.param_miner` + `/param-miner/`) — baseline-vs-probe diffing with a curated 200-word built-in list; three locations (`query` / `body` / `header`); detection signals are reflected sentinel, status change, or body-length delta beyond a configurable tolerance; dependency-injection `send=` kwarg so tests stay offline
...
- [x] Scheduled passive scans (`weblore.scheduler` + `/schedule/`) — persists jobs in `project_state["sched:jobs"]`; uses APScheduler when the new `[schedule]` extra is installed, otherwise a tiny thread-based loop; UI supports start/stop, add (with `interval_s ≥ 30` validation), remove, run-now
- [x] Opt-in update check (`weblore.update_check`) — disabled by default; a single GET to a manifest URL only fires when the user clicks the new button in `/settings`; manifest format is `{latest_version, released, url}`; never auto-checks
- [x] 19 new tests (240 total); all 28 blueprint routes serve 200; CLI `weblore import-har` verified end-to-end (1-entry HAR → 1 history row)
