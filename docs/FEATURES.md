...
| Server-side find-in-body (no JS) | ✅ | History detail, Repeater response, Intercept detail, Scanner finding detail (evidence + payload), WebSocket transcript, Macros detail (JSON definition). See [ACCESSIBILITY.md § Find-in-body](ACCESSIBILITY.md#find-in-body-no-js-aaa-clean). |
