...
# so the first `weblore browser` launch on this machine (or one we ship
...
$argsList = @("-m", "weblore.cli", "prefetch-firefox")
if ($Version) { $argsList += @("--firefox-version", $Version) }
if ($Force)   { $argsList += "--force" }

# Use the same Python that has weblore installed; fall back to `py`.
