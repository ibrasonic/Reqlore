"""Weblore plugin loader.

A plugin is a single ``.py`` file dropped into ``<user>/.weblore/plugins/`` (or
...
class PluginRegistry:
    ...
    def _load_one(self, path: Path) -> PluginRecord:
        modname = f"weblore_plugin_{path.stem}"
...
def default_plugin_dirs() -> list[Path]:
    ...
    return [Path.home() / ".weblore" / "plugins"]
