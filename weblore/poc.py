...
<title>Weblore — CSRF PoC for {_h.escape(url)}</title></head>
<body>
<h1>CSRF PoC</h1>
...
def csrf_fetch_poc(method: str, url: str, headers: list[tuple[str, str]],
    ...
    html = f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<title>Weblore — fetch() CSRF PoC for {_h.escape(url)}</title></head>
<body>
...
def clickjacking_poc(url: str, *, overlay_text: str = "Click here to win!") -> POC:
    ...
    html = f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<title>Weblore — clickjacking PoC for {safe_url}</title>
<style>
