...
Opt-in via ``pip install weblore[impersonate]``. When the library is
present, ``send()`` returns a normal Weblore Response after sending the
    ...
    if not CFFI_AVAILABLE:
        return Response(
            ...
            error="curl_cffi is not installed. "
                  "Run `pip install weblore[impersonate]`.",
        )
