...
Unlike :mod:`weblore.scanner.passive`, active checks **issue HTTP traffic**.
...
Public surface::
    ...
    from weblore.scanner.active import (
...
class OpenRedirectCheck(ActiveCheck):
    ...
    PROBE = "https://weblore-redir.invalid/"
...
class PrototypePollutionCheck(ActiveCheck):
    ...
    def run(self, ctx, send):
        ...
        if not isinstance(obj, dict):
            return
        marker = "weblore_pp_" + secrets.token_hex(3)
        obj.setdefault("__proto__", {"weblore_test": marker})
        ...
        if marker.encode() in pr.response.body[:200_000]:
            yield Finding(
                ...
                host=ctx.host, url=ctx.full_url, request_id=ctx.history_id,
                payload=f'__proto__: {{"weblore_test": "{marker}"}}',
...
class OASTSSRFCheck(ActiveCheck):
    ...
    Requires ``ActiveOptions.oast`` to be a running :class:`weblore.oast.LocalOAST`.
