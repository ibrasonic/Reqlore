...
* Creates the CA on first run, under `~/.weblore/ca/` with 0600 perms.
...
def ensure_ca(ca_dir: Path) -> tuple[Path, Path]:
    ...
    cert_path = ca_dir / "weblore-ca.pem"
    key_path = ca_dir / "weblore-ca.key"
    ...
    name = x509.Name([
        x509.NameAttribute(x509.NameOID.COMMON_NAME, "Weblore Local Root CA"),
        x509.NameAttribute(x509.NameOID.ORGANIZATION_NAME, "Weblore"),
    ])
