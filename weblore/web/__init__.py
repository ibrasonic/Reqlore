...
def create_app(project_path: Path, settings: Settings, *,
    ...
    app.config["WEBLORE_SETTINGS"] = settings
    app.config["WEBLORE_PROXY"] = proxy

    project = Project(project_path)
    app.extensions["weblore_project"] = project
    ...
    def _csrf() -> None:
        ...
        if request.method in ("POST", "PUT", "PATCH", "DELETE"):
            sent = request.form.get("_csrf") or request.headers.get("X-Weblore-CSRF", "")
...
def get_project() -> Project:
    return current_app.extensions["weblore_project"]
