...
from weblore.proxy.matchreplace import MRRule, apply_request, apply_response
...
def test_literal_request_header_replace():
    r = _rule(where="req_header", pattern="User-Agent: old", replacement="User-Agent: weblore")
    h, b = apply_request([r], "example.com",
                         [("User-Agent", "old"), ("Host", "example.com")], b"")
    assert dict(h)["User-Agent"] == "weblore"
