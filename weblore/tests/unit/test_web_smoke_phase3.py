...
from weblore.plugins import reset_registry
...
def app(tmp_path: Path, monkeypatch):
    ...
    from weblore import plugins as plugins_mod
    monkeypatch.setattr(plugins_mod, "default_plugin_dirs",
                         lambda: [tmp_path / "plugins"])
    reset_registry()
    return create_app(tmp_path / "phase3.weblore", Settings(), proxy=None)
...
def test_scanner_finds_and_displays_finding(client, app):
    ...
    proj = app.extensions["weblore_project"]
