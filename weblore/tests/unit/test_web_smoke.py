...
def test_repeater_response_body_has_raw_and_decoded_views(client, monkeypatch):
    ...
    """
    from weblore.engines import Response, Timings
    from weblore.web.blueprints import repeater as rep
...
def test_proxy_intercept_toggle_round_trips(tmp_path):
    ...
    proxy = StubProxy()
    app = create_app(tmp_path / "p.weblore", Settings(), proxy=proxy)
    c = app.test_client()
    ...
    proxy2 = StubProxy()
    app2 = create_app(tmp_path / "p.weblore", Settings(), proxy=proxy2)
...
def test_proxy_controller_set_intercept_mutates_in_place(tmp_path):
    ...
    """
    from weblore.proxy.mitm import ProxyController
    from weblore.storage import Project

    proj = Project(tmp_path / "ictl.weblore")
...
def test_proxy_addon_skips_self_ui_when_intercepting(tmp_path):
    ...
    proj = Project(tmp_path / "skip.weblore")
    ...
    # Hit the Weblore UI directly: must NOT be held.
...
def test_proxy_intercept_count_endpoint_and_watch_attrs(client, app):
    ...
    weblore.js poller knows when to reload.
    """
...
def test_intercept_config_filters_by_method_path_and_excludes():
    ...
    """
    from weblore.proxy.rules import (
...
def test_intercept_config_round_trip_and_persistence(tmp_path):
    ...
    """
    from weblore.proxy.mitm import ProxyController
    from weblore.proxy.rules import InterceptConfig, should_hold_request
    from weblore.storage import Project
    from weblore.web import create_app
    from werkzeug.datastructures import MultiDict

    proj_path = tmp_path / "icfg.weblore"
    ca_dir = tmp_path / "ca"
...
def test_request_only_rule_does_not_hold_responses():
    ...
    must NOT match any response, otherwise the Weblore UI's own 302
...
def test_held_flow_does_not_block_other_flows(tmp_path):
    ...
    operator's own Weblore-UI traffic), which is exactly the symptom
    that prompted this test.
    """
    import asyncio
    from weblore.proxy.mitm import _HistoryAddon
    from weblore.proxy.rules import InterceptConfig
    from weblore.storage import Project

    proj = Project(tmp_path / "concurrent.weblore")
