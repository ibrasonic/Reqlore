...
from reqlore.oast import Interaction, LocalOAST
...
def app(tmp_path: Path):
    return create_app(tmp_path / "p6.rlr", Settings(), proxy=None)
...
def test_active_copy_as_flattens_handlers(tmp_path: Path):
    from reqlore.plugins import PluginRegistry
...
def test_history_copy_as_route_renders_plugin_output(app, tmp_path: Path, monkeypatch):
    from reqlore.plugins import PluginRegistry
    from reqlore.web.blueprints import history as history_bp
