...
def test_har_import_round_trip(tmp_path: Path):
    project = Project(tmp_path / "har.rlr")
    try:
...
def test_har_import_file_wrapper(tmp_path: Path):
    project = Project(tmp_path / "harf.rlr")
    try:
...
def test_scheduler_add_remove_persists(tmp_path: Path):
    project = Project(tmp_path / "sched.rlr")
    try:
        s = Scheduler(project)
...
def test_scheduler_rejects_tiny_interval(tmp_path: Path):
    project = Project(tmp_path / "tiny.rlr")
    try:
        s = Scheduler(project)
        with pytest.raises(ValueError):
...
def test_scheduler_run_now_invokes_scanner(tmp_path: Path):
    project = Project(tmp_path / "rnow.rlr")
    try:
        s = Scheduler(project)
...
def test_update_check_handles_unreachable_url():
    from reqlore.update_check import check
...
def app(tmp_path: Path):
    return create_app(tmp_path / "p7.rlr", Settings(), proxy=None)
